...
const Wrapper:FC<Props> = ({children,client}) => (<QueryClientProvider client={client}>{children}</QueryClientProvider>)

describe('useQuery Hook',()=>{
  it('Should process the test hook because all is well setup', async () => {
    const queryClient = new QueryClient()
    
    const wrapper = ({children}:{children:ReactNode})=>(<Wrapper client={queryClient}>{children}</Wrapper>)
